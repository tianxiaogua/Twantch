...
//			value_2 = 40;
		...
		{
			clock_time_hour[0] = 70; //正常情况下是不会出现六十这样的事件，是无效的事件。判定为无效闹钟就会被删除
		...
		*****************************************/
		...
		if(Number_of_cycles_ == 3 && switch_value == Button_2_pressed) //对分钟调节的功能进行操作
		{
			while(1)
			{
				...
				LCD_ShowIntNum1(5,	110,	value_Year,	2,	BLACK,	LGRAYBLUE,	32); //显示年 //选中以后变成灰蓝色
				...
				if(switch_value == Button_2_pressed)
				{
					LCD_ShowIntNum1(5,	110,	value_Year,	2,	BLACK,	LGRAY,	32);  //显示年 //选中以后变成灰蓝色 //把选中之后的颜色改变回来
		...
		if(Number_of_cycles_ == 4 && switch_value == Button_2_pressed) //对天数调节的功能进行操作
		{
			while(1)
			{
				...
				switch(value_WeekDay)
				{
					...
					case 7:	LCD_ShowChinese(101,		110,	"日",	BLACK,	LGRAYBLUE,	32,0);  break;//显示那一天
				}
