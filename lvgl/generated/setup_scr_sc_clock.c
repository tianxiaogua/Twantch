...
void setup_scr_sc_clock(lv_ui *ui){
	...
	lv_obj_set_pos(ui->sc_clock_label_1, 36, 9);
	...
	//Write codes sc_clock_btn_1
	ui->sc_clock_btn_1 = lv_btn_create(ui->sc_clock);
	lv_obj_set_pos(ui->sc_clock_btn_1, 52, 210);
	lv_obj_set_size(ui->sc_clock_btn_1, 69, 48);
	lv_obj_set_scrollbar_mode(ui->sc_clock_btn_1, LV_SCROLLBAR_MODE_OFF);

	//Write style state: LV_STATE_DEFAULT for style_sc_clock_btn_1_main_main_default
	static lv_style_t style_sc_clock_btn_1_main_main_default;
	if (style_sc_clock_btn_1_main_main_default.prop_cnt > 1)
		lv_style_reset(&style_sc_clock_btn_1_main_main_default);
	else
		lv_style_init(&style_sc_clock_btn_1_main_main_default);
	lv_style_set_radius(&style_sc_clock_btn_1_main_main_default, 5);
	lv_style_set_bg_color(&style_sc_clock_btn_1_main_main_default, lv_color_make(0xc5, 0x34, 0x34));
	lv_style_set_bg_grad_color(&style_sc_clock_btn_1_main_main_default, lv_color_make(0x21, 0x95, 0xf6));
	lv_style_set_bg_grad_dir(&style_sc_clock_btn_1_main_main_default, LV_GRAD_DIR_NONE);
	lv_style_set_bg_opa(&style_sc_clock_btn_1_main_main_default, 255);
	lv_style_set_border_color(&style_sc_clock_btn_1_main_main_default, lv_color_make(0x21, 0x95, 0xf6));
	lv_style_set_border_width(&style_sc_clock_btn_1_main_main_default, 0);
	lv_style_set_border_opa(&style_sc_clock_btn_1_main_main_default, 255);
	lv_style_set_text_color(&style_sc_clock_btn_1_main_main_default, lv_color_make(0xff, 0xff, 0xff));
	lv_style_set_text_font(&style_sc_clock_btn_1_main_main_default, &lv_font_dingliehuobanfont20241217_2_29);
	lv_style_set_text_align(&style_sc_clock_btn_1_main_main_default, LV_TEXT_ALIGN_CENTER);
	lv_obj_add_style(ui->sc_clock_btn_1, &style_sc_clock_btn_1_main_main_default, LV_PART_MAIN|LV_STATE_DEFAULT);
	ui->sc_clock_btn_1_label = lv_label_create(ui->sc_clock_btn_1);
	lv_label_set_text(ui->sc_clock_btn_1_label, "↓");
	lv_obj_set_style_pad_all(ui->sc_clock_btn_1, 0, LV_STATE_DEFAULT);
	lv_obj_align(ui->sc_clock_btn_1_label, LV_ALIGN_CENTER, 0, 0);

	//Write codes sc_clock_btn_2
	ui->sc_clock_btn_2 = lv_btn_create(ui->sc_clock);
	lv_obj_set_pos(ui->sc_clock_btn_2, 52, 74);
	lv_obj_set_size(ui->sc_clock_btn_2, 69, 48);
	...
	lv_style_set_bg_color(&style_sc_clock_btn_2_main_main_default, lv_color_make(0xc5, 0x34, 0x34));
